...
app.post('/api/quote', async (req, res) => {
  try {
    ...
    if (transporter) {
      try {
        try { const _mailRes = await safeSendMail({ to, subject, text, html }); if (!_mailRes.ok) console.warn("Mail not sent:", _mailRes.reason); } catch (e) { console.error("safeSendMail unexpected error:", e && (e.message || e)); }
