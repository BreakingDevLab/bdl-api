...
(function smtpConnectivityCheck() {
  try {
    ...
    dns.lookup(host, { all: true }, (err, addresses) => {
      if (err) {
        console.error('SMTP DNS lookup failed:', err && err.message);
        return;
      }
      ...
      socket.on('connect', () => {
        connected = true;
        console.log(`SMTP TCP connect OK to ${host}:${port}`);
        socket.end();
      });
      socket.on('timeout', () => {
        console.error(`SMTP TCP connect timed out to ${host}:${port}`);
        socket.destroy();
      });
      socket.on('error', (e) => {
        if (!connected) console.error(`SMTP TCP connect error to ${host}:${port}:`, e && e.message);
      });
